		...
		for (int r = 0; r < num_runs; r++) {
			...
			if (n > 1) {
				if (r == num_runs - 1) {
					...
					if (file_output)
						fprintf(output, "cuFFT System: %d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %d benchmark: %d bandwidth: %0.1f\n", dims[0], cuBufferSize / 1024 / 1024, avg_time, std_error, num_iter, (int)(((double)cuBufferSize / 1024) / avg_time), 3*cuBufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time);

					printf("cuFFT System: %d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %d benchmark: %d bandwidth: %0.1f\n", dims[0], cuBufferSize / 1024 / 1024, avg_time, std_error, num_iter, (int)(((double)cuBufferSize / 1024) / avg_time), 3*cuBufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time);
