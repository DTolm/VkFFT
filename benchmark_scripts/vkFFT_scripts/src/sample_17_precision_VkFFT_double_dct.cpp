...
VkFFTResult sample_17_precision_VkFFT_double_dct(VkGPU* vkGPU, uint64_t file_output, FILE* output, uint64_t isCompilerInitialized)
{
	...
	uint64_t benchmark_dimensions[num_benchmark_samples][5] = { {2, 1, 1, 1, 1},{3, 1, 1, 1, 1},{5, 1, 1, 1, 1},{6, 1, 1, 1, 1},{7, 1, 1, 1, 1},{8, 1, 1, 1, 1},{9, 1, 1, 1, 1},{10, 1, 1, 1, 1},{11, 1, 1, 1, 1},{12, 1, 1, 1, 1},{13, 1, 1, 1, 1},{14, 1, 1, 1, 1},
		...
		{8,15319, 1, 1, 2},{8,21269, 1, 1, 2},{8,27283, 1, 1, 2},{8,39829, 1, 1, 2},{8,52733, 1, 1, 2},{8,2000083, 1, 1, 2},{8,4000067, 1, 1, 2},{4,8003869, 1, 1, 2},
		...
		for (int r = 0; r < num_runs; r++) {
			for (int t = 1; t < 5; t++) {
					...
					for (uint64_t l = 0; l < dims[2]; l++) {
						for (uint64_t j = 0; j < dims[1]; j++) {
							for (uint64_t i = 0; i < dims[0]; i++) {
								inputC[i + j * dims[0] + l * dims[0] * dims[1] + k * dims[0] * dims[1] * dims[2]] = (double)(2 * ((double)rand()) / RAND_MAX - 1.0);
				...
				for (uint64_t i = 0; i < numBuf; i++) {
					bufferSize[i] = {};
					bufferSize[i] = (uint64_t)sizeof(double) * configuration.size[0] * configuration.size[1] * configuration.size[2] * configuration.size[3]/ numBuf;
				}
#if(VKFFT_BACKEND_VULKAN)
				...
				*/ // Can specify buffers at launch
				...
				if (resFFT != VKFFT_SUCCESS) return resFFT;
				double* output_VkFFT = (double*)(malloc(sizeof(double) * dims[0] * dims[1] * dims[2] * dims[3]));
