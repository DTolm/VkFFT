...
VkFFTResult sample_2_benchmark_VkFFT_half(VkGPU* vkGPU, uint64_t file_output, FILE* output, uint64_t isCompilerInitialized) 
{
	...
	for (uint64_t n = 0; n < 25; n++) {
		...
		for (uint64_t r = 0; r < num_runs; r++) {
			...
			uint64_t bufferSize = (uint64_t)2 * sizeof(half) * configuration.size[0] * configuration.numberBatches;
#if(VKFFT_BACKEND==0)
			VkBuffer buffer = {};
			...
			*/
			...
			if (n > 0) {
				if (r == num_runs - 1) {
					...
					num_tot_transfers *= 4;
					if (file_output)
						fprintf(output, "VkFFT System: %" PRIu64 " %" PRIu64 "x%" PRIu64 " Buffer: %" PRIu64 " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64 " benchmark: %" PRIu64 " bandwidth: %0.1f\n", (uint64_t)log2(configuration.size[0]), configuration.size[0], configuration.numberBatches, bufferSize / 1024 / 1024, avg_time, std_error, num_iter, (uint64_t)(((double)bufferSize * sizeof(float) / sizeof(half) / 1024) / avg_time), bufferSize / 1024.0 / 1024.0 / 1.024 * num_tot_transfers / avg_time);
