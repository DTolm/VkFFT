...
void user_benchmark_cuFFT(bool file_output, FILE* output, cuFFTUserSystemParameters* userParams)
{
	...
	for (int n = 0; n < 2; n++) {
		...
		for (int r = 0; r < num_runs; r++) {
			...
			case 0:
				if (userParams->R2C){
					cufftPlanMany(&plan, FFTdim, dims, iembed, istride, idist*2, oembed, ostride, odist, CUFFT_R2C, userParams->B);
					cufftPlanMany(&plan2, FFTdim, dims, iembed, istride, idist, oembed, ostride, odist*2, CUFFT_C2R, userParams->B);
				}else{
					cufftPlanMany(&plan, FFTdim, dims, iembed, istride, idist, oembed, ostride, odist, CUFFT_C2C, userParams->B);
				}
				break;
			case 1:
				if (userParams->R2C){
					cufftPlanMany(&plan, FFTdim, dims, iembed, istride, idist*2, oembed, ostride, odist, CUFFT_D2Z, userParams->B);
					cufftPlanMany(&plan2, FFTdim, dims, iembed, istride, idist, oembed, ostride, odist*2, CUFFT_Z2D, userParams->B);
				}else
					cufftPlanMany(&plan, FFTdim, dims, iembed, istride, idist, oembed, ostride, odist, CUFFT_Z2Z, userParams->B);
				break;
			}
