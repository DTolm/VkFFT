...
void launch_benchmark_rocFFT_single_r2c(bool file_output, FILE* output)
{
	...
	for (int n = 0; n < num_benchmark_samples; n++) {
		...
		for (int r = 0; r < num_runs; r++) {
			...
			float totTime = 0;
			uint32_t rocBufferSize = sizeof(float) * 2 * (dims[0]/2 + 1) * dims[1] * dims[2];
			...
			if (n > 0) {
				if (r == num_runs - 1) {
					...
					if (file_output)
						fprintf(output, "rocFFT System: %dx%dx%d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %d benchmark: %d\n", benchmark_dimensions[n][0], benchmark_dimensions[n][1], benchmark_dimensions[n][2], rocBufferSize / 1024 / 1024, avg_time, std_error, num_iter, (int)(((double)rocBufferSize / 1024) / avg_time));

					printf("rocFFT System: %dx%dx%d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %d benchmark: %d\n", benchmark_dimensions[n][0], benchmark_dimensions[n][1], benchmark_dimensions[n][2], rocBufferSize / 1024 / 1024, avg_time, std_error, num_iter, (int)(((double)rocBufferSize / 1024) / avg_time));
