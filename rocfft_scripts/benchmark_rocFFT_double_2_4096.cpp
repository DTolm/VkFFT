...
void launch_benchmark_rocFFT_double_2_4096(bool file_output, FILE* output)
{
	...
	for (int n = 1; n < 4097; n++) {
		...
		for (int r = 0; r < num_runs; r++) {
			...
			if (temp != 1) break;
			dims[1] = pow(2, (uint32_t)log2(64 * 32 * pow(2, 15) / dims[0]));
			...
			if (n > 1) {
				if (r == num_runs - 1) {
					...
					if (file_output)
						fprintf(output, "rocFFT System: %d %d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %d benchmark: %d bandwidth: %0.1f\n", dims[0], dims[1], rocBufferSize / 1024 / 1024, avg_time, std_error, num_iter, (int)(((double)rocBufferSize * sizeof(float) / sizeof(double) / 1024) / avg_time), rocBufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time);
