...
void launch_benchmark_rocFFT_single(bool file_output, FILE* output)
{
	...
	for (int n = 0; n < 26; n++) {
		...
		for (int r = 0; r < num_runs; r++) {
			...
			uint64_t batch = ((3*4096 * 1024.0 * 1024.0) / rocBufferSize > 1000) ? 1000 : (3*4096 * 1024.0 * 1024.0) / rocBufferSize;
			if (batch == 0) batch = 1;
			...
			if (n > 0) {
				if (r == num_runs - 1) {
					...
					if (file_output)
						fprintf(output, "rocFFT System: %" PRIu64 " %" PRIu64 "x%" PRIu64 " Buffer: %" PRIu64 " MB avg_time_per_step: %0.3f ms std_error: %0.3f batch: %" PRIu64 " benchmark: %" PRIu64 "\n", (uint64_t)log2(dims[0]), dims[0], dims[1], rocBufferSize / 1024 / 1024, avg_time, std_error, batch, (uint64_t)(((double)rocBufferSize / 1024) / avg_time));

					printf("rocFFT System: %" PRIu64 " %" PRIu64 "x%" PRIu64 " Buffer: %" PRIu64 " MB avg_time_per_step: %0.3f ms std_error: %0.3f batch: %" PRIu64 " benchmark: %" PRIu64 "\n", (uint64_t)log2(dims[0]), dims[0], dims[1], rocBufferSize / 1024 / 1024, avg_time, std_error, batch, (uint64_t)(((double)rocBufferSize / 1024) / avg_time));
