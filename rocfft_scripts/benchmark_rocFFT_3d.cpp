		...
		for (int r = 0; r < num_runs; r++) {
			...
			if (n > 0) {
				if (r == num_runs - 1) {
					...
					if (file_output)
						fprintf(output, "rocFFT System: %dx%dx%d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f batch: %d benchmark: %d\n", benchmark_dimensions[n][0], benchmark_dimensions[n][1], benchmark_dimensions[n][2], rocBufferSize / 1024 / 1024, avg_time, std_error, batch, (int)(((double)rocBufferSize / 1024) / avg_time));

					printf("rocFFT System: %dx%dx%d Buffer: %d MB avg_time_per_step: %0.3f ms std_error: %0.3f batch: %d benchmark: %d\n", benchmark_dimensions[n][0], benchmark_dimensions[n][1], benchmark_dimensions[n][2], rocBufferSize / 1024 / 1024, avg_time, std_error, batch, (int)(((double)rocBufferSize / 1024) / avg_time));
