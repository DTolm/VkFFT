...
int main()
{
    ...
    const int num_runs = 5;
    int benchmark_dimensions_2D[num_benchmark_samples_2D][4] = { {1024, 1024, 1, 2}, {pow(2,13), 32, 1, 2}, {pow(2,14), 32, 1, 2}, {pow(2,15), 32, 1, 2}, {pow(2,16), 32, 1, 2}, {pow(2,17), 32, 1, 2}, {pow(2,18), 32, 1, 2}, {pow(2,13), pow(2,13), 1, 2},{pow(2,14), pow(2,14), 1, 2} };
        ...
        for (int r = 0; r < num_runs; r++) {
            ...
            for (int j = 0; j < dims[1]; j++) {
                for (int i = 0; i < dims[0]; i++) {
                    inputComplex[i + j * dims[0]].x = i%2-0.5;
