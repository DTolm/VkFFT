...
void main()
{
	...
	int benchmark_dimensions[num_benchmark_samples][4] = { {(uint32_t)pow(2,5), 1, 1, 1},{(uint32_t)pow(2,6), 1, 1, 1},{(uint32_t)pow(2,7), 1, 1, 1},{(uint32_t)pow(2,8), 1, 1, 1},{(uint32_t)pow(2,9), 1, 1, 1},{(uint32_t)pow(2,10), 1, 1, 1},
		...
		{(uint32_t)pow(2,19), 1, 1, 1},{(uint32_t)pow(2,20), 1, 1, 1},{(uint32_t)pow(2,21), 1, 1, 1},{(uint32_t)pow(2,22), 1, 1, 1},{(uint32_t)pow(2,23), 1, 1, 1},{(uint32_t)pow(2,24), 1, 1, 1},{(uint32_t)pow(2,25), 1, 1, 1},{(uint32_t)pow(2,26), 1, 1, 1},{(uint32_t)pow(2,27), 1, 1, 1},
	...
	for (int n = 0; n < num_benchmark_samples; n++) {
		for (int r = 0; r < num_runs; r++) {
			...
			forward_configuration.coalescedMemory = 64;//in bits, for Nvidia compute capability >=6.0 is equal to 32, <6.0 is equal 128. For Intel use 64. Gonna work regardles, but if specified by user correctly, the performance will be higher. 
			...
			for (uint32_t i = 0; i < numBuf; i++) {
				bufferSize[i] = {};
				bufferSize[i] = ((uint64_t)forward_configuration.coordinateFeatures) * 2 * 2 * forward_configuration.size[0] * forward_configuration.size[1] * forward_configuration.size[2] / numBuf;
			}
			...
			uint64_t shift = 0;
			for (uint32_t i = 0; i < numBuf; i++) {
				transferDataFromCPU((float*)(inputC + shift / sizeof(half2)), &buffer[i], bufferSize[i]);
				shift += bufferSize[i];
			}
			...
			half2* output_VkFFT = (half2*)(malloc(sizeof(half2) * dims[0] * dims[1] * dims[2]));

			//Transfer data from GPU using staging buffer.
			shift = 0;
			for (uint32_t i = 0; i < numBuf; i++) {
				transferDataToCPU((float*)(output_VkFFT + shift / sizeof(half2)), &buffer[i], bufferSize[i]);
				shift += bufferSize[i];
			}
